...
function checkForPreCompiled() {
    const majorVer = process.versions.electron.split(".")[0];
    const preCompiledPath = path.resolve(path.join(__dirname, './native/', process.platform, process.arch, `${majorVer}.0.0`, (process.env.EDGE_USE_CORECLR ? 'edge_coreclr' : 'edge_nativeclr')));
