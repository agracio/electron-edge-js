...
function checkForPreCompiled() {
    ...
    if (process.versions.electron) {
        ...
        const preCompiledPath = path.resolve(path.join(__dirname, './native/', process.platform, process.arch, electronVersion, (process.env.EDGE_USE_CORECLR ? 'edge_coreclr' : 'edge_nativeclr')));
...
if (process.env.EDGE_NATIVE) {
    ...
    edgeNative = path.resolve(__dirname, './native/' + process.platform + '/' + process.arch + '/' + electronVersion + '/' + (process.env.EDGE_USE_CORECLR ? 'edge_coreclr.node' : 'edge_nativeclr.node'));
