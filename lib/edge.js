...
if (fs.existsSync(builtEdge)) {
    ...
    checkElectron();
    edgeNative = path.resolve(__dirname, './native/' + process.platform + '/' + process.arch + '/' + process.versions.electron.split(".")[0] + '/' + (process.env.EDGE_USE_CORECLR ? 'edge_coreclr.node' : 'edge_nativeclr.node'));
